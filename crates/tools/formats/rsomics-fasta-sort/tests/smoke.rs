...
fn bin() -> PathBuf { PathBuf::from(env!("CARGO_BIN_EXE_rsomics-fasta-sort")) }
...
fn sort_by_length_desc() {
    let out = Command::new(bin()).args(["-L"]).arg(fixture("unsorted.fa")).output().expect("spawn");
    assert!(out.status.success());
    let s = String::from_utf8(out.stdout).unwrap();
    let first_name = s.lines().next().unwrap();
    assert_eq!(first_name, ">aaa", "longest (8bp) should be first: got {first_name}");
}
