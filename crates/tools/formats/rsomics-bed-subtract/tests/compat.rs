...
fn subtract_matches_bedtools() {
    ...
    write_bed(&a, &["chr1\t100\t500", "chr2\t100\t300"]);
    write_bed(&b, &["chr1\t200\t300", "chr1\t400\t450", "chr2\t150\t250"]);
    ...
    let theirs = Command::new("bedtools")
        ...
        .output()
        .expect("bedtools subtract");
    assert!(theirs.status.success());
    let mut theirs_lines: Vec<String> = String::from_utf8(theirs.stdout)
        .unwrap()
        .lines()
        .map(String::from)
        .collect();
