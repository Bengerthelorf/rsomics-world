...
use rsomics_intervals::{IntervalSet, bed, subtract};
...
const HELP: HelpSpec = HelpSpec {
    ...
    tagline: "Subtract `b` coverage from `a`, emitting residual regions (bedtools subtract equivalent).",
    origin: Some(Origin {
        upstream: "bedtools",
    ...
    sections: &[Section {
        ...
        flags: &[
            FlagSpec {
                ...
                required: true,
                default: None,
                description: "Left input BED (what to keep, minus the b regions)",
                why_default: None,
            },
            FlagSpec {
                short: Some('b'),
                ...
                required: true,
                default: None,
                description: "Regions to remove from `a`",
                why_default: None,
            },
            FlagSpec {
                short: Some('o'),
    ...
    examples: &[Example {
        description: "Subtract blacklist regions from peaks",
        command: "rsomics-bed-subtract -a peaks.bed -b blacklist.bed -o filtered.bed",
    }],
    json_result_schema_doc: None,
};

#[derive(Parser, Debug)]
#[command(name = "rsomics-bed-subtract", disable_help_flag = true)]
struct Cli {
    #[arg(short = 'a', long = "a")]
    a: PathBuf,
    #[arg(short = 'b', long = "b")]
...
fn pipeline(cli: &Cli) -> Result<()> {
    let a_ivs = bed::read(File::open(&cli.a).map_err(RsomicsError::Io)?)?;
    let b_ivs = bed::read(File::open(&cli.b).map_err(RsomicsError::Io)?)?;
    let a_set: IntervalSet = a_ivs.into_iter().collect();
    let b_set: IntervalSet = b_ivs.into_iter().collect();
    let out = subtract(&a_set, &b_set);
